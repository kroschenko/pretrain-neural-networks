...
def get_layers_config_for_dataset(experiment_dataset_name):
    layers_config_selector = {
        DatasetType.MNIST: [
            ...
            # ], "input_dim": (1, 28, 28)},
            {"architecture": [
                [(1, 32, 3, True), [relu, tanh], [bn32]],
                [(32, 32, 3, True), [tanh, relu], [bn32, pooling, dropout_conv]],
                [(32, 64, 3, True), [relu, tanh], [bn64]],
                [(64, 64, 3, True), [tanh, relu], [bn64, pooling, dropout_conv]],
                [(64, 128, 3, False), [relu, tanh], [bn128]],
                [(128, 128, 3, False), [tanh, relu], [bn128, dropout_conv, add_postprocessing]],
                [(1152, 512), [relu, tanh], [bn_fc, dropout]],
                [(512, 10), [softmax]],
            ], "input_dim": (1, 28, 28)},
        ],
        DatasetType.CIFAR10: [
            {"architecture": [
                [(3, 32, 3, True), [relu, relu]],
                [(32, 32, 3, True), [relu, relu], [pooling, dropout_conv]],
                [(32, 64, 3, True), [relu, relu]],
                [(64, 64, 3, True), [relu, relu], [pooling, dropout_conv]],
                [(64, 128, 3, True), [relu, relu]],
                [(128, 128, 3, True), [relu, relu], [pooling, dropout_conv, add_postprocessing]],
                [(2048, 512), [relu, relu], [dropout]],
                [(512, 10), [logsoftmax]],
            ], "input_dim": (3, 32, 32)},
        ],
