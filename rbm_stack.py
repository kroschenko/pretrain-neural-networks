...
class RBMStack:
    ...
    def train_rbm(self, rbm, device, loaders, layer_index, pretrain_type):
        ...
        while not early_stop and epoch < Config.pretraining_epochs:
            ...
            for i, data in enumerate(train_loader):
                ...
                delta_v_thresholds = delta_v_thresholds * momentum + rate / Config.pretraining_batch_size * part_v.sum(
                    0)
                delta_h_thresholds = delta_h_thresholds * momentum + rate / Config.pretraining_batch_size * part_h.sum(
                    0)
    ...
    def train_crbm(self, rbm, device, loaders, layer_index, pretrain_type):
        delta_weights = torch.zeros(rbm.weights.shape).to(device)
        delta_v_thresholds = torch.zeros(rbm.v.shape).to(device)
        delta_h_thresholds = torch.zeros(rbm.h.shape).to(device)
        losses = []
        ...
        while not early_stop and epoch < Config.pretraining_epochs:
            ...
            for i, data in enumerate(train_loader):
                ...
                first_convolution_part = torch.convolution(
                    ...
                    torch.permute(h0, (1, 0, 2, 3)), None, stride=[1, 1], padding=[0, 0], dilation=[1, 1],
                    transposed=False, output_padding=[0, 0], groups=1)
                second_convolution_part = torch.convolution(
                    torch.permute(v1, (1, 0, 2, 3)),
                    torch.permute(part_h, (1, 0, 2, 3)), None, stride=[1, 1], padding=[0, 0], dilation=[1, 1],
                    transposed=False, output_padding=[0, 0], groups=1)
                common_conv_expr = first_convolution_part + second_convolution_part

                delta_weights = delta_weights * momentum + rate / Config.pretraining_batch_size * torch.permute(
                    common_conv_expr, (1, 0, 2, 3))
                delta_v_thresholds = delta_v_thresholds * momentum + rate / (
                            Config.pretraining_batch_size * part_v.shape[2] ** 2) * part_v.sum((0, 2, 3)).reshape(
                    rbm.v.shape)
                delta_h_thresholds = delta_h_thresholds * momentum + rate / (
                            Config.pretraining_batch_size * part_h.shape[2] ** 2) * part_h.sum((0, 2, 3)).reshape(
                    rbm.h.shape)
    ...
    def train_separate_rbm(self, loaders, device, rbm, pretrain_type, layer_index):
        if isinstance(rbm, RBM):
            self.train_rbm(rbm, device, loaders, layer_index, pretrain_type)
        else:
            self.train_crbm(rbm, device, loaders, layer_index, pretrain_type)
